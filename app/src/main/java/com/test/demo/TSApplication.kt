...
import com.chat.invite.WkInviteApplication
...
class TSApplication : MultiDexApplication() {
    ...
    private fun initAll() {
        ...
        WKSecurityApplication.instance.init();//安全隐私模块
        WKMomentsApplication.getInstance().init(this)//朋友圈模块
        WKFavoriteApplication.instance.init();//收藏模块
        WKAdvancedApplication.instance.init()//旗舰模块
        WKImageEditorApplication.getInstance().init()//图片编辑
        WKRichApplication.instance.init()//富文本
        WKPinnedMessageApplication.init()//置顶
        WKStickerApplication.instance.init()//表情动画
        WKCustomerServiceApplication.instance.init()//客服
        WKLabelApplication.instance.init(this)//标签
        WKWalletApplication.getInstance().init(this)//钱包红包
        WKMapApplication.getInstance().init(this)//地图位置模块
        WkInviteApplication.instance.init()//邀请码
    ...
    private fun initApi() {
        ...
        if (TextUtils.isEmpty(apiURL)) {
            // 局域网 API 地址
            apiURL = "http://43.199.60.109:8090"
   ...
   //音视频配置参数 - 局域网配置
    private fun getList(): ArrayList<PeerConnection.IceServer> {
        ...
        val stunServer = PeerConnection.IceServer.builder(
            "stun:43.199.60.109:3478"
        ).createIceServer()
        
        // TURN 服务器 (用于中继)
        val turnServer = PeerConnection.IceServer.builder(
            "turn:43.199.60.109?transport=udp"
