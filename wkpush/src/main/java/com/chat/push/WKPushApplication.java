...
public class WKPushApplication {
    ...
    private void initPush() {
        ...
        // 安全初始化Firebase，防止未配置时崩溃
        try {
            FirebaseApp.initializeApp(mContext.get());
        } catch (Exception e) {
            Log.w("WKPush", "Firebase初始化失败，可能未配置google-services.json: " + e.getMessage());
        }
    ...
    private void getPushToken() {
        ...
        if (statusCode == ConnectionResult.SUCCESS) {
            ...
            try {
                ...
                Log.w("WKPush", "Firebase未初始化，跳过FCM推送: " + e.getMessage());
                // Firebase未初始化，尝试使用国内推送
                initDomesticPush();
            }
        } else {
            initDomesticPush();
