...
class KCustomerServiceModel : WKBaseModel() {
    fun initVisitor() {
        val json = JSONObject()
        ...
        request(createService(KCustomerService::class.java).initVisitor(
    ...
    fun getChatInfo(iChatInfo: IChatInfo) {
        val json = JSONObject()
        json["topic_id"] = 0
        json["appid"] = WKBaseApplication.getInstance().appID
        request(createService(KCustomerService::class.java).getChatInfo(json),
