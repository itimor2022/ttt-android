...
public class FriendModel extends WKBaseModel {
     ...
     * @param topPriority     置顶优先级
     * @param iCommonListener 返回
     */
    public void updateUserTopSetting(String uid, int isTop, int topPriority, final ICommonListener iCommonListener) {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("uid", uid);
        jsonObject.put("is_top", isTop);
        jsonObject.put("top_priority", topPriority);
        request(createService(FriendService.class).updateUserTopSetting(jsonObject), new IRequestResultListener<>() {
            @Override
            public void onSuccess(CommonResponse result) {
                iCommonListener.onResult(result.status, result.msg);
            }
