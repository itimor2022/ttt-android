...
public class ContactsFragment extends WKBaseFragment<FragContactsLayoutBinding> implements OnQuickSideBarTouchListener {
    ...
    protected void initData() {
        ...
        // 过滤掉客服按钮
        List<ContactsMenu> filteredList = new ArrayList<>();
        for (ContactsMenu menu : menuList) {
            if (menu != null && !"customer_service".equals(menu.sid)) {
                filteredList.add(menu);
            }
        }
        contactsHeaderAdapter.setList(filteredList);
        getContacts();
    }
    ...
    private void resetHeaderData() {
        if (isAdded()) {
            ...
            for (int i = 0, size = filteredList.size(); i < size; i++) {
                if (!TextUtils.isEmpty(filteredList.get(i).sid) && filteredList.get(i).sid.equals("friend")) {
                    filteredList.get(i).badgeNum = WKSharedPreferencesUtil.getInstance().getInt(WKConfig.getInstance().getUid() + "_new_friend_count");
                    break;
                }
            }
            contactsHeaderAdapter.setList(filteredList);
