...
public class ChatFragment extends WKBaseFragment<FragChatConversationLayoutBinding> {
    ...
    // 显示后台设置的置顶账号
    private void showTopAccounts() {
        // 获取所有好友
        List<WKChannel> allFriends = WKIM.getInstance().getChannelManager().getWithFollowAndStatus(WKChannelType.PERSONAL, 1, 1);
        if (WKReader.isEmpty(allFriends)) return;
        
        // 检查是否已经在会话列表中
        List<String> existingChannels = new ArrayList<>();
        for (int i = 0; i < chatConversationAdapter.getData().size(); i++) {
            ChatConversationMsg msg = chatConversationAdapter.getData().get(i);
            if (msg != null && msg.uiConversationMsg != null) {
                existingChannels.add(msg.uiConversationMsg.channelID);
            }
        }
        ...
        for (WKChannel channel : allFriends) {
            // 检查是否是置顶账号
            if (channel.top == 1 && !existingChannels.contains(channel.channelID)) {
                WKUIConversationMsg uiConversationMsg = new WKUIConversationMsg();
                uiConversationMsg.channelID = channel.channelID;
                uiConversationMsg.channelType = channel.channelType;
                uiConversationMsg.lastMsgTimestamp = System.currentTimeMillis() / 1000;
                uiConversationMsg.setWkChannel(channel);
                
                ChatConversationMsg chatConversationMsg = new ChatConversationMsg(uiConversationMsg);
...
//        return list;
    ...
    private void sortMsg(List<ChatConversationMsg> list) {
        groupMsg(list);
        List<ChatConversationMsg> officialList = new ArrayList<>();
        ...
        for (int i = 0, size = list.size(); i < size; i++) {
            ...
            if (msg.uiConversationMsg.getWkChannel() != null && msg.uiConversationMsg.getWkChannel().top == 1) {
                ...
                if (WKSystemAccount.isSystemAccount(msg.uiConversationMsg.channelID)) {
                    isOfficial = true;
                } else if (msg.uiConversationMsg.getWkChannel() != null && 
                          (msg.uiConversationMsg.getWkChannel().category.equals("system") || 
                           msg.uiConversationMsg.getWkChannel().category.equals("official"))) {
                    isOfficial = true;
                }
                
                if (isOfficial) {
                    officialList.add(msg);
                } else {
                    normalList.add(msg);
                }
            }
        ...
        // 分别对每个列表按时间戳排序
        Collections.sort(topList, (conversationMsg, t1) -> (int) (t1.uiConversationMsg.lastMsgTimestamp - conversationMsg.uiConversationMsg.lastMsgTimestamp));
        Collections.sort(officialList, (conversationMsg, t1) -> (int) (t1.uiConversationMsg.lastMsgTimestamp - conversationMsg.uiConversationMsg.lastMsgTimestamp));
        Collections.sort(normalList, (conversationMsg, t1) -> (int) (t1.uiConversationMsg.lastMsgTimestamp - conversationMsg.uiConversationMsg.lastMsgTimestamp));
        List<ChatConversationMsg> tempList = new ArrayList<>();
        tempList.addAll(topList);
        tempList.addAll(officialList);
...
//        } else pcLoginTv.setText(appLoginType);
    ...
    private int getInsertIndex(WKUIConversationMsg msg) {
        // 计算官方账号的数量（包括系统账号和category为official的账号）
        int officialCount = 0;
        for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
            ChatConversationMsg item = chatConversationAdapter.getData().get(i);
            boolean isOfficial = false;
            if (WKSystemAccount.isSystemAccount(item.uiConversationMsg.channelID)) {
                isOfficial = true;
            } else if (item.uiConversationMsg.getWkChannel() != null && 
                      (item.uiConversationMsg.getWkChannel().category.equals("system") || 
                       item.uiConversationMsg.getWkChannel().category.equals("official"))) {
                isOfficial = true;
            }
            if (isOfficial) {
                officialCount++;
        ...
        int topCount = 0;
        for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
            if (chatConversationAdapter.getData().get(i).uiConversationMsg.getWkChannel() != null && 
                chatConversationAdapter.getData().get(i).uiConversationMsg.getWkChannel().top == 1) {
                topCount++;
            }
        }
    ...
    private void updateTop(String channelID, byte channelType, int top) {
        if (channelType == WKChannelType.PERSONAL) {
            // 调用服务端置顶设置API
            FriendModel.getInstance().updateUserTopSetting(channelID, top, 1, (code, msg) -> {
                if (code != HttpResponseCode.success) {
                    WKToastUtils.getInstance().showToastNormal(msg);
                }
            });
